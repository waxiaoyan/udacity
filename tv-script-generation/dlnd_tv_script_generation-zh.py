...
def batch_data(words, sequence_length, batch_size):
    ...
    feature_tensor = torch.from_numpy(np.asarray(output))
...
test_text = range(50)
t_loader = batch_data(test_text, sequence_length=20, batch_size=10)
...
# **答案:** (在这里写下)
# 尝试过不同的sequence_length，当sequence_length比较大时，损失减少的比较慢，由于每一行的平均字数为5.5，所以我选择6作为sequence_length的值
# embedding_dim 和 hidden_dim 参考了其他视频和文章中建议值，n_layers我尝试了2，3，当设为2时，损失下降的比较快，所以我选择2,尝试了不同的学习率
# 学习率越小训练越慢，损失减少的越慢
...
trained_rnn = torch.load(save_filename,map_location=lambda storage, loc: storage)
print(trained_rnn)
...
def generate(rnn, prime_id, int_to_vocab, token_dict, pad_value, predict_len=100):
    ...
    for _ in range(predict_len):
        ...
        current_seq = np.roll(current_seq.long(), -1, 1)
